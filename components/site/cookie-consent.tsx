...
export function CookieConsent() {
  ...
  // Focus banner when explicitly reopened via settings event
  useEffect(() => {
    if (forcedOpen) {
  ...
  // Escape key closes the banner when it was reopened and prior consent exists
  useEffect(() => {
    if (!forcedOpen || readConsent() === null) return
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setForcedOpen(false)
    }
